...
public class PersonelManager implements IIslev {
  ...
  private void departmanMaasOrtalamasıHesapla() {
	Map<Departman, Double> departmanMaasOrt = departmanlar.stream().collect(Collectors.toMap(x -> x,x -> x.getDepartmandakiPersoneller().stream().mapToDouble(y -> y.getMaas()).average().orElse(0.0)));
	try {
  ...
  public void mudurlerinDepartmanlari() {
	try {
	  Map<Mudur, List<Departman>> mudurDepartmanMap = personeller.stream().filter(x -> x instanceof Mudur).collect(Collectors.toMap(x -> (Mudur) x,x -> ((Mudur) x).getMudurunDepartmanlari()));
	  for (Entry<Mudur, List<Departman>> mudurDepartmanEntry : mudurDepartmanMap.entrySet()) {
		System.out.println(mudurDepartmanEntry.getKey().getIsim() + " => " + mudurDepartmanEntry.getValue().stream().map(Departman::getIsim).collect(Collectors.toList()));
	  }
	} catch (Exception e) {
	  System.out.println(e.toString());
	}
  ...
  public void ayniGunIseBaslayanlar() {
	Map<LocalDate, List<Personel>> tarhiGorePersonelMap = personeller.stream().collect(Collectors.groupingBy(Personel::getIseGiris));
...
//H-1
	Optional<Personel> arananPersonel = personeller.stream().filter(x -> x.getDepartman().getDepartmandakiPersoneller().stream().anyMatch(y -> y.getId().equals(arananId))).findFirst();
	try {
  ...
  public void deleteById() {
	String arananId = Utility.stringDegerAlma("Aranacak idyi girin");
	Personel arananPersonel = personeller.stream().filter(x -> x.getDepartman().getDepartmandakiPersoneller().stream().anyMatch(y -> y.getId().equals(arananId))).findFirst().orElse(null);
	try {
